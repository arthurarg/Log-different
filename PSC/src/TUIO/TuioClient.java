...
public class TuioClient implements OSCListener {
	 ...
	 * 
	 * @param date
	 *            the time stamp of the OSC bundle
	 * @param message
		...
		if (address.equals("/tuio/2Dcur")) {
			...
			if (command.equals("set")) {
				...
				if (cursorList.get(s_id) == null) {
					...
					if (tcur == null)
						return;
					if ((tcur.xpos != xpos) || (tcur.ypos != ypos)
							|| (tcur.x_speed != xspeed)
							|| (tcur.y_speed != yspeed)
							|| (tcur.motion_accel != maccel)) {

						TuioCursor updateCursor = new TuioCursor(s_id,
								tcur.getCursorID(), xpos, ypos);
				...
				for (int i = 0; i < aliveCursorList.size(); i++) {
					TuioCursor removeCursor = cursorList.get(aliveCursorList
							.elementAt(i));
				...
				if (fseq > 0) {
					...
					if ((fseq >= currentFrame) || ((currentFrame - fseq) > 100))
						...
						lateFrame = true;
				} else if (TuioTime.getSessionTime().subtract(currentTime)
					...
					while (frameEnum.hasMoreElements()) {
						...
						case TuioCursor.TUIO_ADDED:
							...
							if ((cursorList.size() <= maxCursorID)
									&& (freeCursorList.size() > 0)) {
								TuioCursor closestCursor = freeCursorList
										.firstElement();
								Enumeration<TuioCursor> testList = freeCursorList
										.elements();
								while (testList.hasMoreElements()) {
									TuioCursor testCursor = testList
											.nextElement();
									if (testCursor.getDistance(tcur) < closestCursor
											.getDistance(tcur))
							...
							TuioCursor addCursor = new TuioCursor(currentTime,
									tcur.getSessionID(), c_id, tcur.getX(),
									tcur.getY());
						...
						default:
							...
							TuioCursor updateCursor = cursorList.get(tcur
									.getSessionID());
							if ((tcur.getX() != updateCursor.getX() && tcur
									.getXSpeed() == 0)
									|| (tcur.getY() != updateCursor.getY() && tcur
											.getYSpeed() == 0))
								updateCursor.update(currentTime, tcur.getX(),
										tcur.getY());
							else
								updateCursor
										.update(currentTime, tcur.getX(),
												tcur.getY(), tcur.getXSpeed(),
												tcur.getYSpeed(),
												tcur.getMotionAccel());
